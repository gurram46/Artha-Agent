...
Enhanced with AI-driven insights and financial analysis
"""
...
try:
    ...
    logger.warning(f"Authentication endpoints not available: {e}")
    AUTH_ENABLED = False
...
try:
    ...
    logger.warning(f"Portfolio analytics endpoints not available: {e}")
...
try:
    ...
    logger.warning(f"PDF upload endpoints not available: {e}")
    PDF_ENABLED = False
...
try:
    ...
    logger.warning(f"Session management endpoints not available: {e}")
...
# CORS MUST BE ADDED FIRST to ensure proper handling of preflight requests
...
if allowed_origins_env:
    ...
    logger.info("Using development CORS origins. Set ALLOWED_ORIGINS environment variable for production.")
...
# RE-ENABLED WITH CORS COMPATIBILITY AND MODIFIED CSP FOR FRONTEND COMPATIBILITY
# SECURITY MIDDLEWARE ADDED AFTER CORS to avoid interference
try:
    from middleware.security_middleware import SecurityMiddleware
    app.add_middleware(SecurityMiddleware)
    logger.info("CORS-compatible security middleware enabled with frontend-compatible CSP")
except ImportError as e:
...
# Startup handler will be consolidated below - this duplicate handler is removed

# Removed duplicate shutdown handler - keeping the one at line 721
...
class QueryRequest(BaseModel):
    ...
    user_id: Optional[str] = None  # User ID for loading user profile data
...
async def startup_event():
    """Initialize all services and AI components on startup"""
    global chatbot, enhanced_analyst, enhanced_researcher, enhanced_risk_advisor, quick_agent, stock_analyst, CACHE_ENABLED
    
    print("🚀 Initializing Production-Grade Financial AI System...")
    
    # STEP 1: Initialize cache system first (critical infrastructure)
    if CACHE_ENABLED:
        try:
            ...
            print("✅ Cache System Online")
        except Exception as e:
            logger.error(f"Failed to initialize cache system: {e}")
            print(f"⚠️ Cache system failed: {e}")
            CACHE_ENABLED = False
    
    # STEP 2: Initialize AI agents and components
    try:
        ...
        try:
            ...
            print(f"⚠️ Local LLM connection failed: {llm_error}")
        ...
        try:
            ...
            print(f"⚠️ Gemini client initialization failed: {gemini_error}")
        ...
        # Don't exit the server, continue with degraded functionality
...
async def shutdown_event():
    ...
    try:
        ...
        logger.error(f"Failed to cleanup LLM client: {e}")
...
async def stream_query(request: QueryRequest):
    """Stream user financial query response - routes to normal, think, or investment mode"""
    if request.mode == "think":
        return await stream_think_response(request)
    elif request.mode == "investment":
        return await stream_investment_response(request)
    else:
        # Default to normal mode (Gemini 2.5 Flash)
        return await stream_normal_response(request)
...
async def stream_normal_response(request: QueryRequest):
    """Stream normal response using Gemini 2.5 Flash"""
    async def generate_normal_stream():
        try:
            # Load user data
            user_data = await load_user_data(request.user_id) if request.user_id else None
            user_context = ""
            if user_data:
                user_context = f"User: {user_data.get('firstName', 'User')} {user_data.get('lastName', '')}"
                if user_data.get('age'):
                    user_context += f", Age: {user_data['age']}"
                if user_data.get('occupation'):
                    user_context += f", Occupation: {user_data['occupation']}"
                if user_data.get('monthlyIncome'):
                    user_context += f", Monthly Income: ₹{user_data['monthlyIncome']:,}"
                if user_data.get('investmentGoals'):
                    user_context += f", Investment Goals: {', '.join(user_data['investmentGoals'])}"
                if user_data.get('riskTolerance'):
                    user_context += f", Risk Tolerance: {user_data['riskTolerance']}"
            
            # Generate response using Gemini 2.5 Flash
            if not quick_response_agent:
                quick_response_agent = QuickResponseAgent()
            
            # Get financial data for context
            financial_data = await get_financial_data_with_demo_support(demo_mode=request.demo_mode)
            
            response_data = await quick_response_agent.generate_quick_response(
                user_query=request.query,
                financial_data=financial_data
            )
            
            # Convert to generator format for streaming
            ...
            logger.error(f"Normal response streaming error: {e}")
            error_msg = f'Normal response error: {str(e)}'
            yield f"data: {json.dumps({'type': 'error', 'content': error_msg})}\n\n"
            yield f"data: [DONE]\n\n"
...
async def stream_think_response(request: QueryRequest):
    """Stream think response using Gemini 2.5 Pro for deeper analysis"""
    async def generate_think_stream():
        try:
            # Load user data with full context
            user_data = await load_user_data(request.user_id) if request.user_id else None
            user_context = ""
            if user_data:
                user_context = f"User Profile: {user_data.get('firstName', 'User')} {user_data.get('lastName', '')}"
                if user_data.get('age'):
                    user_context += f", Age: {user_data['age']}"
                if user_data.get('occupation'):
                    user_context += f", Occupation: {user_data['occupation']}"
                ...
                if user_data.get('riskTolerance'):
                    user_context += f", Risk Tolerance: {user_data['riskTolerance']}"
                if user_data.get('financialGoals'):
                    user_context += f", Financial Goals: {user_data['financialGoals']}"
            
            # Think mode activation message
            content = "**THINK MODE ACTIVATED**\n Using Gemini 2.5 Pro for deeper analysis..."
            ...
            response_data = await quick_agent.generate_quick_response(
                user_query=request.query,
                financial_data=financial_data
            )
            ...
            logger.error(f"Think response streaming error: {e}")
            error_msg = f'Think response error: {str(e)}'
            yield f"data: {json.dumps({'type': 'error', 'content': error_msg})}\n\n"
            yield f"data: [DONE]\n\n"
    
    return StreamingResponse(
        generate_think_stream(),
        media_type="text/plain",
        headers={
            "Cache-Control": "no-cache",
            "Connection": "keep-alive",
...
async def stream_investment_response(request: QueryRequest):
    """Stream investment advice using SAndeep's multi-agent investment system"""
    async def generate_investment_stream():
        try:
            from sandeep_investment_system.sandeep_api_integration import sandeep_api
            
            newline = "\n"
            
            # Investment mode activation
            investment_content = f'💰 **INVESTMENT AGENT ACTIVATED**{newline}🤖 Multi-agent investment analysis system'
            yield f"data: {json.dumps({'type': 'log', 'content': investment_content})}\n\n"
            await asyncio.sleep(0.1)
            ...
            # Initialize SAndeep system if needed
            if not sandeep_api.initialized:
                yield f"data: {json.dumps({'type': 'log', 'content': '🔧 Initializing investment agents...'})}\n\n"
                sandeep_api._initialize_system()
                await asyncio.sleep(0.2)
            
            # Agent analysis simulation
            agents = [
                {'name': 'Data Analyst', 'emoji': '📊', 'task': 'Market research and data analysis'},
                {'name': 'Trading Analyst', 'emoji': '📈', 'task': 'Investment strategy formulation'},
                {'name': 'Risk Analyst', 'emoji': '⚖️', 'task': 'Risk assessment and mitigation'},
                {'name': 'Execution Analyst', 'emoji': '🎯', 'task': 'Portfolio optimization'}
            ]
            
            for agent in agents:
                 agent_msg = f"{agent['emoji']} {agent['name']}: {agent['task']}..."
                 yield f"data: {json.dumps({'type': 'log', 'content': agent_msg})}\n\n"
                 await asyncio.sleep(0.3)
            
            # Get investment response from SAndeep system
            mcp_data = {
            ...
            yield f"data: {json.dumps({'type': 'status', 'content': 'Investment analysis complete'})}\n\n"
            yield f"data: [DONE]\n\n"
            
        except ImportError as e:
            error_msg = f"Investment system not available. Please install required dependencies: {str(e)}"
            yield f"data: {json.dumps({'type': 'error', 'content': error_msg})}\n\n"
            yield f"data: [DONE]\n\n"
        except Exception as e:
            logger.error(f"❌ Investment response streaming failed: {e}")
            error_msg = f"Investment analysis failed: {str(e)}"
            yield f"data: {json.dumps({'type': 'error', 'content': error_msg})}\n\n"
            yield f"data: [DONE]\n\n"
...
# @app.get("/api/stream/future-projection")  # DISABLED - Money Truth Engine removed
# async def stream_future_projection():
#     """Stream AI-driven future wealth projection with typing effect"""
#     # Money Truth Engine functionality has been removed
#     pass
...
async def stream_goal_reality():
    ...
    raise HTTPException(status_code=500, detail="Goal reality check disabled - MoneyTruthEngine removed")
    ...
    async def generate():
        try:
            ...
            # Run analysis - DISABLED: MoneyTruthEngine removed
            # insights = await money_truth_engine.life_goal_simulator(mcp_data)
            insights = {'goal_analysis': 'Goal analysis functionality temporarily disabled'}
...
async def stream_money_personality():
    ...
    # if not money_truth_engine:
    #     raise HTTPException(status_code=500, detail="MoneyTruthEngine not initialized")
    raise HTTPException(status_code=500, detail="MoneyTruthEngine functionality disabled")
    ...
    async def generate():
        try:
            ...
            # Run analysis - DISABLED: MoneyTruthEngine removed
            # # insights = await money_truth_engine.analyze_money_personality(mcp_data)  # DISABLED: MoneyTruthEngine removed
            insights = {'personality_analysis': 'Money personality analysis functionality temporarily disabled'}
...
async def get_money_personality_fallback():
    ...
    # if not money_truth_engine:
    #     raise HTTPException(status_code=500, detail="MoneyTruthEngine not initialized")
    raise HTTPException(status_code=500, detail="Money personality analysis disabled - MoneyTruthEngine removed")
...
# @app.post("/api/real-time-insights")  # DISABLED - Money Truth Engine removed
# async def get_real_time_insights(query_request: QueryRequest):
#     """Get real-time AI insights for specific user queries"""
#     # Money Truth Engine functionality has been removed
#     pass

# @app.post("/api/portfolio-health")  # DISABLED - Money Truth Engine removed
...
# @app.post("/api/risk-assessment")  # DISABLED - Money Truth Engine removed
# async def get_risk_assessment(demo: bool = False):
#     """Get comprehensive risk assessment using Money Truth Engine"""
#     # Money Truth Engine functionality has been removed
#     pass

# @app.post("/api/trip-planning")  # DISABLED - Money Truth Engine removed
...
# @app.post("/api/trip-planning/chat")  # DISABLED - Money Truth Engine removed
    ...
    try:
        while True:
            ...
            try:
                ...
                # Get real-time insights - DISABLED: MoneyTruthEngine removed
