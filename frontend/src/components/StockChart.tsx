...
export default function StockChart({ symbol, onClose, embedded = false }: StockChartProps) {
  ...
  const contentClass = embedded 
    ? "w-full" 
    : "bg-white rounded-3xl p-8 max-w-7xl w-full max-h-[90vh] overflow-y-auto";
          ...
          <div className="flex items-start justify-between mb-8">
            <div className="space-y-2">
              <div className="flex items-center space-x-4">
                <h2 className="text-3xl font-bold text-slate-900">{stockData.name}</h2>
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                  {stockData.sector}
                </span>
              </div>
              <p className="text-lg text-slate-600">{stockData.symbol}</p>
              <div className="flex items-baseline space-x-4">
                <h3 className="text-4xl font-bold text-slate-900">{formatPrice(stockData.currentPrice)}</h3>
